...
class Page_m extends MY_Model
{
		...
		if ($uri === null)
		{
			$page = $this->db
				->where('is_home', true)
                                //gjergji
                                ->where('day =', date('N'))
                                ->where('MAKETIME('.date('H').','.date('i').',0) BETWEEN MAKETIME(hour_from, minute_from, 0) AND MAKETIME(hour_to, minute_to, 0)')
				//end gjergji
                                ->get('pages')
				->row();
		}
	...
	public function create($input, $stream = false)
	{
		...
		$id = $this->insert(array(
			...
			'order'				=> now(),
                        'day'			=> isset($input['day']) ? $input['day'] : '0',
                        'hour_from'			=> isset($input['hour_from']) ? $input['hour_from'] : '0',
                        'minute_from'		=> isset($input['minute_from']) ? $input['minute_from'] : '0',
                        'hour_to'			=> isset($input['hour_to']) ? $input['hour_to'] : '0',
                        'minute_to'		=> isset($input['minute_to']) ? $input['minute_to'] : '0',                    
