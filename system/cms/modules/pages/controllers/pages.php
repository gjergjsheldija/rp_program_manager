...
class Pages extends Public_Controller
{
	...
	public function _page($url_segments)
	{
		...
		if ( !$page or 
                     ($page->status == 'draft' and ! $this->permission_m->has_role(array('put_live', 'edit_live'))) //or 
                     //(
                     //    $page->day != date('N') //and 
                         //$page->hour_from >= date('H') and $page->hour_to <= date('H') and
                         //$page->minute_from >= date('i') and $page->minute_to <= date('i')                        
                     // )
                   )
		{
